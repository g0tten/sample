...
public class Dissect extends Processor {

	private static final String PROJECT_PATH = ModelManager.getWorkspaceAbsolutePath() + File.separator + "sample.gotten";
	private static final String INITIAL_WORKLOAD_PATH = ModelManager.getEclipseHomePath() + File.separator + "workspace3" + File.separator + "dissect-cf-examples-master" + File.separator + "anon_jobs.gwf";
				...
				if (outputFile.exists() == true) {
					...
					case "time":
						...
						while (scanner.hasNextLine()) {
							...
							if (line.startsWith("Total simulation time: ")) {
								...
								strValue = line.substring(line.indexOf("Total simulation time: ") + "Total simulation time: ".length(), line.length()).trim();
								strValue = strValue.replace(",", ".");
								if (strValue.indexOf(".") != -1) {
									strValue = strValue.substring(0,strValue.indexOf(".") );
								}
								long value = Long.parseLong(strValue);
								System.out.println("[dissect] simulation time: " + value);
								values.add(value);
							}
						}
						reader.close();
					...
					case "energy":
						...
						while (scanner.hasNextLine()) {
							...
							search = "Total power consumption: ";
							...
							if (nStart != -1) {
								...
								strValue = strValue.replace(",", ".");
								System.out.println("[dissect] energyvalue: " + strValue);
	...
	private boolean generateWorkload(Resource model) {
		...
		try {
			...
			for (EObject workload : workloads) {
				...
				pathToAdd = pathToAdd.replace(File.separator + "workload", File.separator + "workload" + File.separator + this.getID() + File.separator + model.getURI().lastSegment().toString().replace(".model", "") + File.separator);
				...
				if (workloadPaths.contains(pathToAdd + File.separator + workloadName) == false) {
					workloadPaths.add(pathToAdd + File.separator + workloadName);
					workloadIndex++;
				}
	...
	public int execute(List<Resource> models) {
		...
		System.out.println("[dissect] - running models!");
		args[0] = "--standalone";
		for (Resource model : models) {
			String modelName = model.getURI().toFileString().substring(model.getURI().toFileString().lastIndexOf(File.separator + "model" + File.separator) + (File.pathSeparator + "model" + File.separator).length(), model.getURI().toFileString().length());
